...
export function useDarkStoreOS(storeId: string) {
  ...
  useEffect(() => {
    setInventory(INITIAL_INVENTORY.map(item => ({ ...item, currentStock: Math.floor(Math.random() * 60) + 20 })));
    setRevenue(Math.floor(Math.random() * 5000) + 1000);
    setEvents([{ id: 'init', time: new Date().toLocaleTimeString(), msg: `System Link Established for ${storeId}`, type: 'alert' }]);
  ...
  const tick = useCallback(() => {
    ...
    setInventory(prev => prev.map(item => {
      ...
      // Auto-restock logic for critical items
      if (newStock < 5) {
        ...
        setTimeout(() => {
          setEvents(e => [{ 
            ...
            msg: `AGENT: Auto-restock ${item.name} (+${restockQty}). ${poNumber}`, 
            type: 'restock' 
          }, ...e].slice(0, 50));
      ...
      // If depletion happened, add to revenue
      if (depletion > 0.1) {
    ...
    if (now.getSeconds() % 15 === 0) {
      setSalesTrends(prev => {
        ...
        return [...prev.slice(1), { time: nextTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), sales: Math.floor(Math.random() * 50 * multiplier) }];
      });
    }
  }, [storeId]);
